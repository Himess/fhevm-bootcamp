...
describe("MultiUserVault", function () {
  ...
  it("should withdraw within balance", async function () {
    const encDeposit = await fhevm
      ...
      .add64(1000)
      .encrypt();
    await (await contract.connect(alice).deposit(encDeposit.handles[0], encDeposit.inputProof)).wait();

    const encWithdraw = await fhevm
      .createEncryptedInput(contractAddress, alice.address)
      .add64(400)
      .encrypt();
    await (await contract.connect(alice).withdraw(encWithdraw.handles[0], encWithdraw.inputProof)).wait();
  ...
  it("should isolate users deposits", async function () {
    ...
    const clearAlice = await fhevm.userDecryptEuint(FhevmType.euint64, handleAlice, contractAddress, alice);
    expect(clearAlice).to.equal(1000n);

    const handleBob = await contract.connect(bob).getMyDeposit();
    const clearBob = await fhevm.userDecryptEuint(FhevmType.euint64, handleBob, contractAddress, bob);
