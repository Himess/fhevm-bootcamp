...
FHEVM does not have a built-in range function. You generate a full-range random value and then use `FHE.rem()` (encrypted modulo) to constrain it. The Solidity `%` operator cannot be used because the value is encrypted and not accessible as a plaintext integer.
</details>
