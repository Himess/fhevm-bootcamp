...
/// @title PrivateVoting - Module 06: Encrypted voting with private ballots
contract PrivateVoting is ZamaEthereumConfig {
    ...
    constructor(uint8 _candidateCount) {
        ...
        // Initialize all tally counters to encrypted 0
    ...
    function vote(externalEuint8 encryptedVote, bytes calldata inputProof) external {
        ...
        // For each candidate, check if vote matches and increment tally
        ...
        // Re-set ACL for all updated tallies
        FHE.allowThis(_tally0);
        FHE.allowThis(_tally1);
        FHE.allowThis(_tally2);
        FHE.allowThis(_tally3);
    ...
    /// @notice Reveal tallies publicly (only after voting closes)
