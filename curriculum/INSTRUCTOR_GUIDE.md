...
- **Note:** In v0.9+, GatewayConfig is no longer needed. Use `FHE.makePubliclyDecryptable()` for decryption.
...
- Introduce the encrypted types: `ebool`, `euint8`, `euint16`, `euint32`, `euint64`, `euint128`, `euint256`, `eaddress`.
...
**Discussion Questions:**
1. "Why does fhEVM offer so many integer sizes (euint8 through euint256)? Why not just euint256 for everything?"
...
**Key Teaching Points:**
- fhEVM provides encrypted randomness via `FHE.randEuintXX()` functions.
...
| FHE.randEuintXX functions | 20 min |
